...
const eraseBtn = document.querySelector("#erased-button");
...
const updateTodo = (text) => {
  ...
  todos.forEach((todo) => {
    ...
    if (todoTitle.innerText === oldInputValue) {
      ...
      editTodoStatusLocalStorage(oldInputValue, text);
    }
  });
};
...
const filterTodos = (filterValues) => {
  const todos = document.querySelectorAll(".todo");
...
const editTodoStatusLocalStorage = (oldInputValue, newInputValue) => {
  const todos = getTodosLocalStorage();

  todos.map((todo) =>
    todo.texto === oldInputValue ? (todo.texto = newInputValue) : null
  );
